...
//:
//: This example shows the use of the optional type cast operator (`as?`) to check whether a shape in an array of shapes is a circle or a square. You increment the count of the `squares` and `circles` variables by one each time the corresponding shape is found, printing the values at the end.
//:
